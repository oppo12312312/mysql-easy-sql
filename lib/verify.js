 ...
 * @LastEditTime: 2019-07-19 17:00:49
 */
'use strict';
const verifyType = {
  where: 'object',
...
module.exports = {
   ...
   */
  verifySelect(tableName, where, columns, orders) {
    const all = { tableName, where, columns, orders };
  ...
  verifyPage(pageNum, pageSize) {
    ...
    this.verifyDataType(page);
    for (const key in page) {
      if (!/^\d+$/.test(page[key] + '') || page[key] < 1) {
  ...
  verifyValue(tableName, column, value) {
    ...
    if ((types.string.indexOf(columnType) > -1 || types.enum.indexOf(columnType) > -1) && valueType !== 'string') {
      throw new Error(`${info} 必须是字符串类型`);
    }
    if ((types.dateTime.indexOf(columnType) > -1) && valueType !== 'number') {
      throw new Error(`${info} 必须是uni数字时间戳`);
    }
  },
