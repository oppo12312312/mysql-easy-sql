 ...
 * @LastEditTime: 2019-07-19 16:34:10
 */
...
module.exports = {
  ...
  getSqlWhere(where, otherName, tableName = '') {
    ...
    for (const key in where) {
      ...
      let dbType = '';
      if (tableName) {
        columnSql = `${otherName}.\`${dbColumn}\``;
        dbType = dbInfo.getColumnType(lintTableName, lineColumn);
      } else {
        columnSql = `${otherName}.${lineColumn}`;
        dbType = this.getColumnTypeByValue(values);
      }
      const inValueArr = [];
      switch (type) {
        case 'number':
        ...
        case 'array':
          values.forEach(value => {
            const sqlValue = this.getSqlValue(value, dbType);
            inValueArr.push(sqlValue);
          });
          wheres.push(`${columnSql}  in (${inValueArr.join(' , ')})`);
          break;
        default:
      }
    }
  ...
  getColumnTypeByValue(values) {
    const type = this.valueType(values);
    // const now = Date.parse(new Date());
    // const interval = 1000 * 60 * 60 * 24 * 365;
    // const start = now;
    let re = '';
    switch (type) {
      case 'number':
      ...
      case 'object':
        for (const key in values) {
          re = this.getColumnTypeByValue(values[key]);
          break;
        }
        break;
      case 'array':
        if (values.length > 0) {
          re = this.getColumnTypeByValue(values[0]);
        }
        break;
      default:
    }
  ...
  getSqlSet(data, tableName, otherName) {
    ...
    for (const key in data) {
      ...
      const dbColumn = dbInfo.getColumn(lintTableName, lineColumn);
